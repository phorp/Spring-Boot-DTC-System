...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>配件名称：</div>
        <el-input v-model="formSearch.peijianmingcheng" placeholder="配件名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>配件分类：</div>
        <el-input v-model="formSearch.peijianfenlei" placeholder="配件分类" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0 0 0","margin":"0","background":"none","flex":"1","width":"calc(100% - 250px)","order":"6"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 0px","fontSize":"14px","color":"#666","background":"none","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 10px","margin":"0 2% 20px 0","overflow":"hidden","background":"#fff","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.peijiantupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-if="item.peijiantupian && item.peijiantupian.substr(0,4)=='http'" :src="item.peijiantupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.peijiantupian?item.peijiantupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.peijiantupian?item.peijiantupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.peijianmingcheng}}</div>
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">订单金额:{{item.dingdanjine}}</div>
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.chukuriqi}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`peijianjilu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/peijianjiluDetail', query: params});
      },
