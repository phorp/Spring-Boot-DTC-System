...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>配件品牌：</div>
        <el-input v-model="formSearch.peijianpinpai" placeholder="配件品牌" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('peijianxinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","display":"inline-block","outline":"none","borderRadius":"20px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/peijianxinxiAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/peijianfenlei/peijianfenlei').then(res => {
			if (res.data.code == 0) {
				this.peijianfenleiOptions = res.data.data;
				this.selectOptionsList.push({name:'配件分类',list:this.peijianfenleiOptions,tableName: 'peijianfenlei',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.peijianpinpai != '') searchWhere.peijianpinpai = '%' + this.formSearch.peijianpinpai + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`peijianxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/peijianxinxiDetail', query: params});
      },
