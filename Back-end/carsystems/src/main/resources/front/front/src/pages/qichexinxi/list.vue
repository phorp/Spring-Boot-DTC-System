...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>汽车名称：</div>
        <el-input v-model="formSearch.qichemingcheng" placeholder="汽车名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	  ...
	  <el-button v-if=" true " :style='{"cursor":"pointer","border":"1px solid #3693a450","padding":"0px 8px","margin":"0px 10px 0 10px","color":"#fff","borderRadius":"20px","background":"linear-gradient(to right,#f67536,#369ff6),#369ff6","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="getList(1, curFenlei)"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-search"></i>查询</el-button>
	  <el-button v-if="btnAuth('qichexinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","display":"inline-block","outline":"none","borderRadius":"20px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/qichexinxiAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"padding":"0 2%","margin":"0","overflow":"hidden","textAlign":"right","background":"none","width":"100%","order":"4"}'>
		<el-button :style='{"padding":"2px 15px","margin":"0 5px","borderColor":"#eccc19","borderRadius":"0px","background":"none","borderWidth":"0px","borderStyle":"solid"}' @click="sortClick('price')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#3693a4"}' class="icon iconfont icon-chujia13" v-if="sortType!='price'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#3693a4"}' class="icon iconfont icon-jiantou06" v-else-if="sortType=='price'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#3693a4"}' class="icon iconfont icon-jiantou07" v-else-if="sortType=='price'&&sortOrder=='asc'"></span>
		...
		<el-button :style='{"border":"0px solid #ddd","padding":"2px 15px","margin":"0 5px","borderRadius":"0px","background":"none"}' @click="sortClick('clicknum')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#3693a4"}' class="icon iconfont icon-xiaoliang17" v-if="sortType!='clicknum'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#3693a4"}' class="icon iconfont icon-jiantou06" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#3693a4"}' class="icon iconfont icon-jiantou07" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0 0 0","margin":"0","background":"none","flex":"1","width":"calc(100% - 250px)","order":"6"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 0px","fontSize":"14px","color":"#666","background":"none","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 10px","margin":"0 2% 20px 0","overflow":"hidden","background":"#fff","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.qichetupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-if="item.qichetupian && item.qichetupian.substr(0,4)=='http'" :src="item.qichetupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.qichetupian?item.qichetupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.qichetupian?item.qichetupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.qichemingcheng}}</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/qichepinpai/qichepinpai').then(res => {
			if (res.data.code == 0) {
				this.qichepinpaiOptions = res.data.data;
				this.selectOptionsList.push({name:'汽车品牌',list:this.qichepinpaiOptions,tableName: 'qichepinpai',check: -1})
			}
		});
	    this.cheliangfenleiOptions = '轿车,轿跑,suv,mpv,其他车型'.split(',');
		this.selectOptionsList.push({name:'车辆分类',list:this.cheliangfenleiOptions,tableName: 'cheliangfenlei',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getFenlei() {
		this.$http.get('qichepinpai/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'qichepinpai'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.cheliangfenlei != '') searchWhere.cheliangfenlei = this.formSearch.cheliangfenlei;
