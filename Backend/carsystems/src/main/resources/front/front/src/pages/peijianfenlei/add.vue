    ...
    >
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="配件分类" prop="peijianfenlei">
            <el-input v-model="ruleForm.peijianfenlei" 
                placeholder="配件分类" clearable :disabled=" false  ||ro.peijianfenlei"></el-input>
          </el-form-item>
  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          peijianfenlei: [
            { required: true, message: '配件分类不能为空', trigger: 'blur' },
          ],
        },
		centerType: false,
      };
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`peijianfenlei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('peijianfenlei/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`peijianfenlei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
