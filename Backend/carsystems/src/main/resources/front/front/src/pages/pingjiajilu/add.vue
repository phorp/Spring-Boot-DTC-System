    ...
    >
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务单号" prop="fuwudanhao">
            <el-input v-model="ruleForm.fuwudanhao" 
                placeholder="服务单号" clearable :disabled="true  ||ro.fuwudanhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="评价图片" v-if="type!='cross' || (type=='cross' && !ro.pingjiatupian)" prop="pingjiatupian">
            <file-upload
            tip="点击上传评价图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="评价图片" prop="pingjiatupian">
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="服务评分" prop="fuwupingfen">
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="评价日期" prop="pingjiariqi">
              <el-date-picker
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`pingjiajilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('pingjiajilu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`pingjiajilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
