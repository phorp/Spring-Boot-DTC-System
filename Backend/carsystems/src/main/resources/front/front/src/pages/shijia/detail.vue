  ...
  <div>
    <div
      ...
      <el-breadcrumb :separator="'='"
        ...
        <el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/shijia">
...
<!--          <div class="item"
          ...
          <div class="btn" :style='{"width":"100%","padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
            <el-button
                ...
                v-if="btnAuth('shijia','修改')" @click="editClick">修改
            </el-button>
            <el-button
                ...
                v-if="btnAuth('shijia','删除')" @click="delClick">删除
            </el-button>
            <el-button
                :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#f53400","borderRadius":"0px","background":"none","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}'
                v-if="btnAuth('shijia','私聊')&&detail.id!=mid" @click="chatClick">联系TA
            </el-button>
            <el-button
                ...
                v-if="btnAuth('shijia','服务接单')"
                @click="onAcross('jinduxinxi','','','yuyueqingkuang','该预约单已受理！','已受理,未受理'.split(',')[0])"
                type="warning">服务接单
            </el-button>
            <el-button
                :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#f53400","borderRadius":"0px","background":"none","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}'
                v-if="btnAuth('shijia','审核')" @click="onSH" type="warning">回复
            </el-button>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async onAcross(acrossTable, crossOptAudit, crossOptPay, statusColumnName, tips, statusColumnValue, type = 1) {
      localStorage.setItem('crossTable', `shijia`);
    ...
    backClick() {
      ...
      this.$router.push({path: '/index/shijia', query: params});
    },
    // 下载
    download(file) {
      if (!file) {
    ...
    // 修改
    editClick() {
      this.$router.push(`/index/shijiaAdd?type=edit&&id=${this.detail.id}`);
    },
    // 删除
    async delClick() {
      await this.$confirm('是否删除此预约信息？')
          .then(_ => {
            this.$http.post('shijia/delete', [this.detail.id]).then(async res => {
              if (res.data.code == 0) {
                this.$message({
                  type: 'success',
                  message: '删除成功!',
