...
<div :style='{"width":"100%","padding":"30px 18% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知标题" prop="tongzhibiaoti">
            <el-input v-model="ruleForm.tongzhibiaoti" 
                placeholder="通知标题" clearable :disabled=" false  ||ro.tongzhibiaoti"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="图片信息" v-if="type!='cross' || (type=='cross' && !ro.tupianxinxi)" prop="tupianxinxi">
            <file-upload
            tip="点击上传图片信息"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="图片信息" prop="tupianxinxi">
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知日期" prop="tongzhiriqi">
              <el-date-picker
          ...
          <el-form-item :style='{"width":"48%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="通知内容" prop="tongzhineirong">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="通知内容"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`xiaoxitixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('xiaoxitixing/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`xiaoxitixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
